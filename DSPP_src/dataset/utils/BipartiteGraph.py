...
class BipartiteGraph(object):
    ...
    def add_pre_graph(self, pre_graph, decay):
        self.graph_interaction_number = 0
        for user in pre_graph.keys():
            if user not in self.bigraph:
                self.bigraph[user] = {}
            for item in pre_graph[user].keys():
                if pre_graph[user][item] * decay < 0.1:
                    continue
    ...
    def random_neg_with_pad(self,user, pos):
        if pos == self.opt["num_item"] - 1:
            return pos
    ...
    def random_neg_pos(self,user, pos):
        if pos == self.opt["num_item"] - 1:
            return pos
        ...
        if len(self.bigraph[user]) < 3:
            ...
            items = list(self.bigraph[user].keys())
            n = 5
            while n:
                n -= 1
                rand = random.randint(0, len(items) - 1)  # del pad
                if items[rand] != pos: # maybe contain other pos item!
                    return items[rand]
            return self.opt["num_item"] - 1


    def random_list_multi_neg_with_some_pos(self, user, positive_list, nega_number):
        ret = []
        for _ in positive_list:
            res = []
            for i in range(nega_number):
                if i >= nega_number-self.opt["pos_neg"]:
                    res.append(self.random_neg_pos(user, _))
                else :
                    res.append(self.random_neg_with_pad(user, _))
            ret.append(res)
        return ret
    ...
    def decay(self, factor):
        for user in self.bigraph.keys():
            for item in self.bigraph[user].keys():
                self.bigraph[user][item] = self.bigraph[user][item] * factor
                if self.bigraph[user][item] < 0.1:
                    del self.bigraph[user][item]
